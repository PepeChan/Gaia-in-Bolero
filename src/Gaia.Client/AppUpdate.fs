...
let private markReviewNeededForParsedAtomExclusion sourcePhiId atomKind atomText resetImpacts affectedCandidateIds model =
    let trigger = "Model Fitting item excluded"
    let reason = "A parsed interpretation was excluded from active Model Fitting; dependent governance or realization should be reviewed."

    let basisMarkedModel =
        resetImpacts
        |> List.fold
            (fun workingModel (impact: ParseAmendmentBasisDecisionResetImpact) ->
                workingModel
                |> appendReviewNeededMark
                    reviewTargetKindSigmaBasisItemDecision
                    impact.BasisItemKey
    ...
    if equalsReviewText atomKind "Host" then
        governanceMarkedModel
        |> markHostRealizationReviewNeeded sourcePhiId trigger reason atomText
    elif equalsReviewText atomKind "Function" then
        governanceMarkedModel
        |> markFunctionRealizationReviewNeeded sourcePhiId trigger reason atomText
    else
        governanceMarkedModel

let formatParsedAtomRetirementLedgerDetail sourcePhiId atomKind atomText provenance basisReviewCount candidateReviewCount =
    [
        ...
        "Basis decision review marks: " + string basisReviewCount
        "Candidate decision review marks: " + string candidateReviewCount
           ...
           || String.IsNullOrWhiteSpace(cleanedAtomText) then
            ...
            | Some parse ->
                ...
                if not (atomListContains cleanedAtomText currentAtoms) then
                    ...
                    let candidateDecisionReviewCount =
                        model.candidateDecisions
                    ...
                    let candidateReviewSummary =
                        match candidateDecisionReviewCount with
                        | 0 -> "No candidate group decision needed review marking."
                        | 1 -> "Marked 1 candidate group decision Review Needed."
                        | count -> "Marked " + string count + " candidate group decisions Review Needed."
                    ...
                    |> appendLedgerEvent
                        ...
                        (formatParsedAtomRetirementLedgerDetail
                            ...
                            candidateDecisionReviewCount)
                    |> markReviewNeededForParsedAtomExclusion
                        cleanedSourcePhiId
                        cleanedAtomKind
                        cleanedAtomText
                        resetImpacts
